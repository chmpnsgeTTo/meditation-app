...
app.get('/api/my-posts', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM posts WHERE user_id = $1 ORDER BY created_at DESC', [req.user.userId]);
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: 'Ошибка' });
  }
...
app.get('/api/courses', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT c.*, 
        COALESCE(ucp.completed_lessons, 0) as user_completed,
        COALESCE(ucp.is_completed, false) as user_is_completed
      FROM courses c
      LEFT JOIN user_course_progress ucp ON ucp.course_id = c.id AND ucp.user_id = $1
      ORDER BY 
        CASE c.difficulty
          WHEN 'Начинающий' THEN 1
          WHEN 'Средний' THEN 2
...
app.get('/api/courses/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const progressRes = await pool.query(`
      SELECT completed_lessons, is_completed FROM user_course_progress 
      WHERE user_id = $1 AND course_id = $2
    `, [req.user.userId, id]);
    
    const completedLessonsRes = await pool.query(`
      SELECT lesson_id FROM user_lesson_completions 
      WHERE user_id = $1 AND lesson_id IN (SELECT id FROM lessons WHERE course_id = $2)
    `, [req.user.userId, id]);
    
    const completedLessonIds = completedLessonsRes.rows.map(r => r.lesson_id);
    
    res.json({
      ...course,
      lessons: lessonsRes.rows,
      progress: progressRes.rows[0] || { completed_lessons: 0, is_completed: false },
      completedLessonIds
    });
  } catch (err) {
...
app.post('/api/courses/:courseId/lessons/:lessonId/complete', authMiddleware, async (req, res) => {
  ...
  console.log(`📝 Отметка урока: user=${userId}, course=${courseId}, lesson=${lessonId}`);
  
  try {
    // 1. Проверяем, что урок существует и принадлежит курсу
    const lessonCheck = await pool.query('SELECT id FROM lessons WHERE id = $1 AND course_id = $2', [lessonId, courseId]);
    if (lessonCheck.rows.length === 0) {
      return res.status(404).json({ error: 'Урок не найден' });
    }
    
    // 2. Добавляем запись о завершении урока
    await pool.query(`
    ...
    // 3. Получаем количество завершённых уроков
    ...
    console.log(`✅ Завершено уроков: ${completedCount}`);
    
    // 4. Получаем общее количество уроков в курсе
    ...
    if (totalLessons === 0) {
      return res.status(400).json({ error: 'В курсе нет уроков' });
    }
    
    const isCompleted = completedCount >= totalLessons;
    
    // 5. Обновляем прогресс (исправленный запрос без неоднозначности)
    if (isCompleted) {
      await pool.query(`
    ...
    console.log(`💾 Прогресс сохранён: ${completedCount}/${totalLessons}, завершён: ${isCompleted}`);
    
    res.json({ 
      completedCount, 
      totalLessons, 
      isCompleted,
      progressPercent: (completedCount / totalLessons) * 100
    });
    
  } catch (err) {
    console.error('❌ Ошибка сохранения прогресса:', err);
    res.status(500).json({ error: 'Ошибка сохранения прогресса', details: err.message });
  }
});
...
// Подключаем middleware для проверки прав администратора
...
// Удалить любого пользователя (для админа)
app.delete('/api/admin/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
  const { id } = req.params;
  try {
    // Проверяем, что не удаляем самого админа
